...
$itemList = new Item;
$countItem = $itemList -> countItemListOfUser($user -> userId);
...
if($countItem > 0) {
	foreach($itemList -> getItemListOfUser($user -> userId) as $r) {
		...
		echo('<div class="line-item">');
			...
			echo('<div class="line-item-content'.(($r['premium'] > time()) ? ' line-item-premium' : '').'">');
				...
				if($mobile == 1) echo('<div style="height: 10px;"></div>');
				echo('<a href="'.$url -> getUrl('item/edit', false, '/'.$r['item_id']).'" class="button button-2">Zarządzaj</a> ');
				if($r['premium'] > time() || $r['validity'] > time()) {
					if($mobile == 1) echo('<div style="height: 10px;"></div>');
					echo('<a href="'.$url -> getUrl('item/stat', false, '/'.$r['item_id']).'" class="button button-2">Statystyki</a> ');
				}
				if($mobile == 1) echo('<div style="height: 10px;"></div>');
				echo('<div class="'.(($mobile == 0) ? 'section-top-right' : '').'">');
				if($r['premium'] > time()) {
					echo('Pakiet Premium ważny do '.dateTimeFormat($r['premium']));
				} elseif($r['validity'] > time()) {
					echo('Pakiet Standard ważny do '.dateTimeFormat($r['validity']));
				} else {
					echo('Pakiet Darmowy');
				}
				echo('</div>');
